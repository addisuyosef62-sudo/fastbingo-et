...
import { getJetXCrashQueue, setJetXCrashQueue, clearJetXCrashQueue } from "@/lib/jetx";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <Header
        ...
        currentView={view}
      />
      <main className="flex-1 px-3 py-2 w-full max-w-3xl md:max-w-5xl mx-auto">
      ...
      <nav className="sticky bottom-0 grid grid-cols-4 bg-card">
...
function Header({
  ...
  return (
    <header className="sticky top-0 z-10 bg-card">
      <div className="w-full max-w-3xl md:max-w-5xl mx-auto flex items-center gap-2 px-3 py-2">
        ...
        <div className="flex-1 flex items-center justify-center gap-2">
          <button
            ...
            className={`rounded-md overflow-hidden border-2 ${currentView === "game" ? "border-primary shadow-[0_0_0_2px_hsl(var(--primary)/0.3)]" : "border-border"} hover:border-primary transition`}
            title="Bingo"
            aria-label="Bingo"
          >
            <img src={bingoBanner} alt="Bingo" className="h-10 w-auto block" />
          </button>
          <Link
            to="/jetx"
            className="rounded-md overflow-hidden border-2 border-border hover:border-primary transition"
            title="Crash Game"
            aria-label="Crash Game"
          >
            <img src={aviatorBanner} alt="Crash" className="h-10 w-auto block" />
          </Link>
        </div>
...
function GameView({
  ...
  const playStartSound = () => playOneShot("/sounds/Game_Start.mp3");
  ...
  const handlePlay = () => {
    if (locked || status.drawing) return;
    if (!cartella.some((n) => n > 0)) { setErr(t("errPick")); return; }
    if (bet < MIN_BET) { setErr(`Minimum bet is ${MIN_BET} ETB`); return; }
    const fresh = getCurrentUser();
    if (!fresh || fresh.balance < bet) { setErr(t("errInsufficient")); return; }
    updateUser({ ...fresh, balance: fresh.balance - bet, games: (fresh.games ?? 0) + 1 });
    lockedTicketRef.current = { cartella: [...cartella], bet };
    setLocked(true);
    logActivity({ username: user.username, type: "play", detail: `bet ${bet} ETB · cartella #${cartellaId ?? "-"}` });
    onChange();
  };
  ...
  const runDraw = async () => {
    ...
    // Pre-call intro sound must finish before the first bingo number is called.
  ...
  const pickCartella = (id: number) => {
    if (status.drawing || locked) return;
    setCartellaId(id);
    setCartella(cartellaById(id));
    setDaubedManual(new Set([12]));
  };
  const clearCartella = () => {
    if (status.drawing || locked) return;
      ...
      <div className="space-y-3">
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          {STAKES.map((s) => {
            ...
            return (
              <div key={s} className="grid grid-cols-[70px_50px_50px_1fr_65px] sm:grid-cols-[110px_70px_70px_1fr_90px] items-center px-2 sm:px-3 py-2 sm:py-3 border-b border-border last:border-b-0">
                <div>
  ...
  return (
    <div className="space-y-2">
      {err && (
        <div className="err-banner flex items-center gap-2 px-3 py-2 text-sm">
      ...
      <div className="flex items-center justify-between text-[11px] px-1">
        <span className="text-muted-foreground tracking-widest">PATTERN</span>
        <span className="text-primary font-bold uppercase">{pattern.label}</span>
      </div>

      {/* Prize / Countdown panel — visible always, including during calling */}
      <div className="prize-panel">
        <div>
          <span className="chip">ደራሽ</span>
          <div className="text-white font-black text-2xl leading-none mt-1">
            {Math.floor((players.reduce((s, p) => s + p.bet, 0) + bet) * (1 - HOUSE_CUT))} ETB
          </div>
        </div>
        <div className="text-right">
          <div className="text-[10px] text-muted-foreground tracking-widest">
            {status.drawing ? "ጨዋታው ተጀምሯል" : "ጨዋታው ሊጀምር ነው"}
          </div>
          <div className="text-white font-black text-2xl leading-none mt-1">
            {status.drawing
      ...
      {/* DRAWING MODE */}
      {status.drawing ? (
        <div className="bg-card border rounded-lg p-4 min-h-[320px] relative overflow-hidden flex flex-col gap-3">
          <div className="rings" />

          {/* Top: Cartella (left) + Called ball / recent balls (right) */}
          <div className="relative grid grid-cols-[150px_1fr] gap-3 items-start">
            <div className="justify-self-start">
              {lockedTicketRef.current ? (
                ...
                <div className="text-[10px] text-muted-foreground text-center">No ticket</div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {drawn.length > 0 && (
                <div className="relative grid grid-cols-5 gap-1 justify-items-center">
                  {(["B","I","N","G","O"] as const).map((L) => {
                    const colBalls = drawn.filter((n) => bingoLetter(n) === L);
                    return (
                      <div key={L} className="flex flex-col gap-1 items-center">
                        <div
                          className="w-7 h-7 rounded-md flex items-center justify-center text-white font-black text-xs"
                          style={{ background: BINGO_COL_BG[L] }}
                        >
                          {L}
                        </div>
                        {colBalls.map((n, i) => (
          ...
          {/* Below: full-width 90-number board */}
          <div className="relative cart-grid lined">
            {Array.from({ length: 75 }, (_, i) => i + 1).map((n) => {
              ...
              return (
                <div
                  key={n}
                  className={`cart-cell ${called ? "called" : ""} ${isCurrent ? "current" : ""}`}
                >
                  {n}
                </div>
              );
        ...
        <div className={`bg-card border-2 rounded-lg p-4 space-y-3 ${revealing.won ? "border-primary pulse-glow" : "border-border"}`}>
          ...
          <CartellaGrid
            ...
            onTap={() => {}}
          />
          <div className="flex flex-wrap gap-1 justify-center pt-2 border-t border-border">
        ...
        <>
            ...
            <div className="flex-1 min-w-0">
              {cartellaId === null ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-card border rounded-lg p-3 relative overflow-hidden">
                    <div className="grid grid-cols-1 sm:grid-cols-[180px_1fr] gap-3 items-start">
                      ...
                      >
                        ...
                        <label className="flex items-center gap-1 mt-2 text-[10px] text-muted-foreground">
                          <input
                            type="checkbox"
                            checked={autoDaub}
          ...
          {/* Bet controls */}
          <div className="grid grid-cols-[1fr_auto_auto] gap-2 items-center pt-1">
            <div className="flex items-center bg-card border rounded-md">
              <button onClick={() => setBet((b) => Math.max(MIN_BET, b - 10))} className="p-2 text-muted-foreground"><Minus size={16} /></button>
              <input value={bet} onChange={(e) => setBet(Math.max(MIN_BET, Number(e.target.value) || MIN_BET))} className="flex-1 bg-transparent text-center font-bold w-12" />
              <button onClick={() => setBet((b) => b + 10)} className="p-2 text-muted-foreground"><Plus size={16} /></button>
            </div>
            <button onClick={() => setBet((b) => Math.max(MIN_BET, b * 2))} className="px-3 py-2 bg-card border rounded-md text-primary text-sm font-bold">{t("double")}</button>
            <button onClick={() => setBet(Math.max(MIN_BET, Math.floor(user.balance) || MIN_BET))} className="px-3 py-2 bg-card border rounded-md text-primary text-sm font-bold">{t("max")}</button>
          </div>

          <button
            onClick={handlePlay}
            disabled={locked}
            className="play-btn w-full py-3.5 rounded-md font-bold text-base disabled:opacity-80"
          >
            {locked ? t("waiting") : t("placeBet")}
          </button>
        </>
      )}
...
function JetXCrashQueueAdmin() {
  ...
  const save = () => {
    const nums = text
      ...
      .slice(0, 10);
    setJetXCrashQueue(nums);
    setQueueState(nums);
    setText(nums.map((n) => n.toFixed(2)).join(", "));
    toast.success(`Saved ${nums.length} crash points`);
  };
  const clear = () => {
    clearJetXCrashQueue();
    setQueueState([]);
    setText("");
    toast.message("Cleared — random crashes resume");
  };
  return (
    <div className="bg-card border rounded-md p-3 space-y-2">
      <div className="text-sm font-bold text-primary">JetX Crash Queue (next 10 rounds)</div>
      <div className="text-[11px] text-muted-foreground">
        Manually set the crash multiplier for the next rounds. Comma-separated,
        up to 10 values (e.g. <code>1.20, 2.50, 1.05, 10.00</code>). Plane will
        crash at each value in order, then resume random.
      </div>
      <input
        value={text}
        onChange={(e) => setText(e.target.value)}
        placeholder="1.20, 2.50, 1.05, 10.00"
        className="w-full bg-background border rounded-md px-2 py-1.5 text-sm"
      />
      <div className="grid grid-cols-2 gap-2">
        <button onClick={save} className="bg-primary text-primary-foreground font-bold py-1.5 rounded-md text-sm">Set Queue</button>
        <button onClick={clear} className="bg-destructive text-destructive-foreground font-bold py-1.5 rounded-md text-sm">Clear</button>
      </div>
      {queue.length > 0 && (
        <div className="flex flex-wrap gap-1 pt-1">
          {queue.map((m, i) => (
            <span key={i} className="px-2 py-0.5 rounded-full text-[11px] font-bold tabular-nums bg-secondary text-secondary-foreground">
              {i + 1}. {m.toFixed(2)}x
            </span>
          ))}
        </div>
